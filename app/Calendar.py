...
class Calendar(GoogleApi):
    ...
    with open("./secrets/calendar_id") as secret:
        CALENDAR_ID = secret.read().strip()
    ...
    def get_events_for_next_week(self):
        ...
        next_monday = datetime.date.today() + datetime.timedelta(
            days=(0 - datetime.date.today().weekday()) % 7
        )
    ...
    def event_intersects(self, event_dict):
        ...
        for event in events:
            ...
            new_event = datetime.datetime.strptime(
                event_dict["date"] + " " + event_dict["time"], "%Y-%m-%d %H:%M:%S"
            )
    ...
    def get_rents_for_month(self, month):
        ...
        start = datetime.datetime.strptime(
            "01-" + str(month) + "-" + str(current_year), "%d-%m-%Y"
        ).date()
        end = datetime.datetime.strptime(
            str(calendar.monthrange(current_year, month)[1])
            + "-"
            + str(month)
            + "-"
            + str(current_year),
